...
# parameters
city_coords  = "[49.7084, 0.202]" # Etretat
...
cam_angles_init = copy(cam_angles)
...
# convert pix remarkable points to geo local, and to world csr
# xyz_remarkables_from_pix = pix_2_world(df_corresp_pts_remarquables[['U', 'V', 'elevation']], georef_params)
#
# # convert geo points remarkable in local coordinate system, and to pix
# u_remarkables_from_geo, v_remarkables_from_geo = world_2_pix(
...
with ui.matplotlib(figsize=(28, 12), tight_layout=True) as plot:
    ...
    ax1.scatter(df_corresp_pts_remarquables['U_undist'], df_corresp_pts_remarquables['V_undist'], c='b', s=6,
                                label='remarkables')
    sc_uv = ax1.scatter(u_remarkables_from_geo, v_remarkables_from_geo, c='cyan', s=6, label='remarquables from geo')
    ax1.legend(fontsize=16)
    ax2 = plot.figure.add_subplot(122)
    ax2.imshow(data_ortho, extent=extent_ortho, aspect='equal')
    ax2.axes.get_xaxis().set_ticks([])
    ax2.axes.get_yaxis().set_ticks([])
    ax2.scatter(df_corresp_pts_remarquables['easting'], df_corresp_pts_remarquables['northing'],
                                c='b', s=6, label='remarkables')
    sc_geo = ax2.scatter(xyz_remarkables_from_pix[0, :], xyz_remarkables_from_pix[1, :], c='cyan', s=6,
                label='remarkables from pix')
    ax2.legend(fontsize=16)
    ax2.set_xlim([298245, 298295])
    ax2.set_ylim([5509940, 5509990])
    plot.update()  # <- indispensable pour afficher dès le début

def update_plot(georef_params, new_angle, i_angle, origin, cam_angles_init):
    ...
    sc_uv.set_offsets(np.c_[u_remarkables_from_geo, v_remarkables_from_geo])
    sc_geo.set_offsets(np.c_[xyz_remarkables_from_pix[0, :], xyz_remarkables_from_pix[1, :]])
    ax1.set_title(f"second angle updated from {cam_angles_init[1]} to {new_angle}")
    plot.update()
...
ui.label('angle 1')
i_angle = 0
slider = ui.slider(min=cam_angles_init[i_angle] - 0.5, max=cam_angles_init[i_angle] + 0.5, value=cam_angles[i_angle],
                   step=0.1, on_change=lambda e: update_plot(georef_params, e.value, i_angle, origin, cam_angles_init))

ui.label('angle 2')
i_angle = 1
slider = ui.slider(min=cam_angles_init[1] - 2, max=cam_angles_init[1] + 2, value=cam_angles[1], step=0.1,
                   on_change=lambda e: update_plot(georef_params, e.value, i_angle, origin, cam_angles_init))
