...
from read_inputs import read_ortho, read_img, read_gcps, read_tif_mnt, read_litto3d_pts
...
def toggle_mnt():
    ...
    if sc_geo_mnt is None:
        sc_uv_mnt_from_geo = ax1.scatter(mnt_u_from_geo, mnt_v_from_geo, c= mnt_z, cmap='RdYlBu_r', s=2,
                                         label='mnt drone from geo', alpha=0.2)
...
def toggle_litto3d():
    global sc_geo_litto3d
    global sc_uv_litto3d_from_geo
    if sc_geo_litto3d is None:
        sc_uv_litto3d_from_geo = ax1.scatter(u_litto, v_litto, c= df_litto3d_pts['z'], cmap='RdYlBu_r', s=2,
                                         label='litto3d', alpha=0.2)
        sc_geo_litto3d = ax2.scatter(df_litto3d_pts['x'], df_litto3d_pts['y'], c=df_litto3d_pts['z'], cmap='RdYlBu_r',
                                     s=6, label='litto3d')
        ax1.set_xlim([2100, 2390])
        ax1.set_ylim([1100, 1330])
        set_sc_axis_limits(ax1, 0, width, 0, height, reverse_yaxis=True, margin=25)
        ...
        # Supprimer le scatter existant
        sc_geo_litto3d.remove()
        sc_geo_litto3d = None
        sc_uv_litto3d_from_geo.remove()
        sc_uv_litto3d_from_geo = None
        reset_sliders()
    ax2.legend(fontsize=16)
    optimized_update_plot()
...
def update_plot():
    ...
    # Calcul des points litto3d pix
    if sc_uv_litto3d_from_geo is not None:
        u_litto, v_litto = world_2_pix(df_litto3d_pts[['x', 'y', 'z']], georef_params)
        sc_uv_litto3d_from_geo.set_offsets(np.c_[u_litto, v_litto])
...
def reset_view():
    set_sc_axis_limits(ax1, 0, width, 0, height, reverse_yaxis=True)
    set_sc_axis_limits(ax2, extent_ortho[0], extent_ortho[1], extent_ortho[2], extent_ortho[3], margin=20)
    optimized_update_plot()
...
f_litto3d_semi_pts = '/home/florent/Projects/Etretat/litto3d/0495_6965/NHDF-MAR_FRA_0498_6961_2016-2018_L93_RGF93_IGN69/Semis_points_Sol/NHDF-MAR_FRA_0498_6961_PTS_2016-2018_L93_RGF93_IGN69_selection_wide_area.xyz'
# NB: f_litto3d_semi_pts est issu d'une operation qgis de sélection dans le semi de points à l'intérieur d'un polygon,
# exporté en UTM31N
...
# read litto3d
df_litto3d_pts, u_litto, v_litto = read_litto3d_pts(f_litto3d_semi_pts, georef_params)
...
with ui.button_group().classes('mx-auto'):
    ...
    # bouton litto3d
    button_litto3d_pts = ui.button('Litto 3D', on_click=toggle_litto3d)
    button_litto3d_pts.style('width: 200px; height: 20px; font-size: 15px;')

    # bouton reset view
    button_reset_view = ui.button('Reset view', on_click=reset_view)
    button_reset_view.style('width: 200px; height: 20px; font-size: 15px;')
